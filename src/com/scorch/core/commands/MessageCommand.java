...
public class MessageCommand extends BukkitCommand {
	...
	public boolean execute(CommandSender sender, String commandLabel, String[] args) {
		...
		if (!sender.hasPermission("scorch.command.message.cross-server.ignorefriends")) {
			if (!ScorchCore.getInstance().getModule("FriendModule", FriendModule.class).areFriends(player.getUniqueId(),
					target.getUniqueId())) {
				MSG.cTell(sender, "noperm");
				return true;
			}
		...
		if (!ScorchCore.getInstance().getCommunicationModule().isOnline(target.getUniqueId())) {
			MSG.tell(sender, "That player is offline");
			return true;
		}
