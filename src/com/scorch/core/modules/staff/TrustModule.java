...
public class TrustModule extends AbstractModule {
	...
	public double getTrust(UUID uuid) {
		...
		List<Report> reportsAgainst = rm.getReportsAgainst(uuid), reportsSubmitted = rm.getReports(uuid);

		int accurateReports = reportsSubmitted.stream()
				.filter(r -> r.isHandled() && r.getResolutionType() == ResolutionType.CONFIRMED)
		...
		for (Report report : reportsSubmitted.stream().filter(Report::isHandled).collect(Collectors.toList())) {
			switch (report.getResolutionType()) {
			case ABUSE:
				reportScore -= 20;
				break;
			case CONFIRMED:
		...
		double proV = Math.max(minutes + gamesPlayed * 5 + friends * 2.5 + accurateReports * 10, 1);
		double punishV = Math.max(punishmentScore + directAlts * 20 + otherAlts * 10, 1);
		double reportV = Math.max(reportsAgainst.size() * 20 + (reportsSubmitted.size() - accurateReports) * 10, 1);

		return (proV + reportScore) / (punishV + reportV);
	}
