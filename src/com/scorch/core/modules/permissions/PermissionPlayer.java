 ...
 * permissions a player might have besides their groups permissions.
 *
...
public class PermissionPlayer {
     ...
     * <br><b>THIS SHOULD ONLY BE CALLED ONCE!</b></br>
     * @param player the player
    ...
    public PermissionAttachment createAttachment (Player player) {
        ...
        getGroups().forEach(group ->{
            ((PermissionModule)ScorchCore.getInstance().getModule("PermissionModule"))
                    .getGroup(group).getPermissions().forEach(node -> attachment.setPermission(node, true));
        });
        return attachment;
    }
